$( function () {
	...
	$('.btn-new-motion-graphic').on('click', function(event) {
		event.preventDefault();

		$('#exampleModal').modal('show');
	});

	var t = $('#motion-graphic-table').DataTable({
		"processing" : true,
		"language": {
		...
		"order": [], 
		"ajax": {
			"url": base_url + "item_graphic/view_motion_graphic",
			"type": "POST"
		},
		"searchDelay" : 750,
		"columnDefs"	: [{
		...
		$.ajax({
			...
			success: function(data){
				if (data.type != 'done'){
					...
					$("#motion_graphic_type").val ( data.msg[0].motion_graphic_type );
					$("#motion_graphic_name").val ( data.msg[0].motion_graphic_name );
