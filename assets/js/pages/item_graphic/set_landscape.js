$( function () {
	...
	$('.btn-new-set-landscape').on('click', function(event) {
		event.preventDefault();

		$('#exampleModal').modal('show');
	});

	var t = $('#set-landscape-table').DataTable({
		"processing" : true,
		"language": {
		...
		"order": [], 
		"ajax": {
			"url": base_url + "item_graphic/view_set_landscape",
			"type": "POST"
		},
		"searchDelay" : 750,
		"columnDefs"	: [{
		...
		$.ajax({
			...
			success: function(data){
				if (data.type != 'done'){
					...
					$("#set_landscape_type").val ( data.msg[0].set_landscape_type );
					$("#set_landscape_name").val ( data.msg[0].set_landscape_name );
