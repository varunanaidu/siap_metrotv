$( function () {
	...
	$('.btn-new-operational').on('click', function(event) {
		event.preventDefault();

		$('#exampleModal').modal('show');
	});

	var t = $('#operational-table').DataTable({
		"processing" : true,
		"language": {
		...
		"order": [], 
		"ajax": {
			"url": base_url + "item_graphic/view_operational",
			"type": "POST"
		},
		"searchDelay" : 750,
		"columnDefs"	: [{
		...
		$.ajax({
			...
			success: function(data){
				if (data.type != 'done'){
					...
					$("#operational_type").val ( data.msg[0].operational_type );
					$("#operational_name").val ( data.msg[0].operational_name );
