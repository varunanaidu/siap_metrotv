$( function () {
	...
	$('.btn-new-immersive').on('click', function(event) {
		event.preventDefault();

		$('#exampleModal').modal('show');
	});

	var t = $('#immersive-table').DataTable({
		"processing" : true,
		"language": {
		...
		"order": [], 
		"ajax": {
			"url": base_url + "item_graphic/view_immersive",
			"type": "POST"
		},
		"searchDelay" : 750,
		"columnDefs"	: [{
		...
		$.ajax({
			...
			success: function(data){
				if (data.type != 'done'){
					...
					$("#immersive_type").val ( data.msg[0].immersive_type );
					$("#immersive_name").val ( data.msg[0].immersive_name );
