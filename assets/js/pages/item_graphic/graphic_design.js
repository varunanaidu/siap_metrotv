$( function () {
	...
	$('.btn-new-graphic-design').on('click', function(event) {
		event.preventDefault();

		$('#exampleModal').modal('show');
	});

	var t = $('#graphic-design-table').DataTable({
		"processing" : true,
		"language": {
		...
		"order": [], 
		"ajax": {
			"url": base_url + "item_graphic/view_design_graphic",
			"type": "POST"
		},
		"searchDelay" : 750,
		"columnDefs"	: [{
		...
		$.ajax({
			...
			success: function(data){
				if (data.type != 'done'){
					...
					$("#graphic_design_type").val ( data.msg[0].graphic_design_type );
					$("#graphic_design_name").val ( data.msg[0].graphic_design_name );
